;;; concur-lazy.el --- Async slot resolution via promises and futures -*- lexical-binding: t; -*-
;;
;;; Commentary:
...
(cl-defstruct
    (concur-lazy-opts
     (:constructor nil)
     (:constructor concur-lazy-opts-create (&key auto-fetch timeout retry logger)))
...
(defun concur-lazy-opts-new (options)
...
(defun concur-lazy--expand (form &optional replacements)
...
(defun concur-lazy--maybe-init-slot (obj slot opts fn-body callback)
...
CALLBACK is passed to the FN-BODY if used interactively or eagerly."
  ...
  (unless (and obj slot fn-body)
    (error "concur-lazy--maybe-init-slot: must provide OBJ, SLOT, and FN-BODY"))

  (log! "Initializing slot `%s` on `%s` with auto-fetch=%s" slot obj (concur-lazy-opts-auto-fetch opts))
  (let* ((thunk (lambda ()
...
(defun concur-lazy--await-slot (obj slot opts fn-body callback)
...
- If uninitialized: invoke `concur-lazy--maybe-init-slot` and possibly force it.
...
OPT supports `:auto-fetch`, `:retry`, `:timeout`, etc."
  (declare (indent defun))
  (unless (and obj slot fn-body)
    (error "concur-lazy--await-slot: must provide OBJ, SLOT, and FN-BODY"))

  (log! "Awaiting slot `%s` on `%s` with auto-fetch=%s" slot obj (concur-lazy-opts-auto-fetch opts))
  (let* ((val (slot-value obj slot))
    ...
    (cond
     ...
     (t
      (let ((result (concur-lazy--maybe-init-slot obj slot opts fn-body callback)))
        (cond
...
;;;###autoload
(cl-defmacro concur-lazy-generic-accessor! (field &optional type)
...
Checks that TYPE is a known `cl-defstruct` and FIELD is a valid slot."
 ...
 (log! "Defining generic accessor for field `%s` in type `%s`" field type)
...
This macro expands placeholders in BODY (e.g., `<>`) to refer to the object instance."
  ...
  (log! "Defining lazy accessor for slot `%s` in type `%s`" slot type)
  (let* ((type-sym (or (and type (if (eq (car-safe type) 'quote) (cadr type) type)) 'generic)) 
         ...
         (expanded-body (concur-lazy--expand body)))
    ;; Macro expansion
    `(progn
...
- Hooks into the constructor (once) to auto-fetch the slot after creation."
  ...
  (log! "Defining async lazy accessor for slot `%s` in type `%s`" slot type)
  (let* ((type (or type 'generic))
         ...
         (replacements `((<> . ,obj)
                         (<cb> . ,callback)
                         (<callback> . ,callback)))
         (fn-body (concur-lazy--expand fn replacements)))
    `(progn
       ;; Async getter
       (defun ,getter (,obj)
         ...
         (let* ((,opts-sym (if (concur-lazy-opts-p ,options)
                               ,options
                             (concur-lazy-opts-new ,options))))
           (concur-lazy--await-slot ,obj ',slot ,opts-sym ',fn-body
       ...
       (defun ,initfn (,obj)
         ...
         (let ((,opts-sym (if (concur-lazy-opts-p ,options)
           ...
           (when (concur-lazy-opts-auto-fetch ,opts-sym)
             (concur-lazy--maybe-init-slot ,obj ',slot ,opts-sym ',fn-body
                                    (lambda (resolved-value)
                                      (setf (slot-value ,obj ',slot) resolved-value))))))
...
- FN: Expression to compute the slot value, may use `<>` for object."
  ...
  (log! "Defining safe synchronous lazy accessor for slot `%s` in type `%s`" slot type)
  (let* ((fn-name (intern (format "%s-%s" type slot)))
         (predicate (intern (format "%s-p" type)))
         (obj (gensym "obj"))
         (replacements `(( "<>" . ,obj)))
         (fn-expanded (concur-lazy--expand fn replacements)))
    `(defun ,fn-name (,obj)
...
;;;###autoload
(defmacro concur-lazy-accessors! (&rest args)
...
The generated getter attempts to use a slot-specific accessor if defined, and falls back to direct slot lookup."
  ...
  (log! "Defining lazy accessors for type `%s`" (plist-get args :type))
  (let* ((type (plist-get args :type))
    ...
    (dolist (field eager)
      (let ((fn-name (intern (format "%s-%s" type-name field))))
        (push
         `(concur-lazy-generic-accessor! ,field ,type)
         forms)))
