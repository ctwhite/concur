...
;; and should terminate their execution if the token has been canceled. This
...
(defvar concur--cancel-token-hooks (ht-create)
  "Hash table mapping cancel tokens to their cancellation callback functions.")
...
Returns:
`t` if the token is active, `nil` if canceled or invalid."
  (and (concur-cancel-token-p token)
       (concur-cancel-token-active token)))

;;;###autoload
(defun concur:cancel-token-check (token)
  "Check if TOKEN is canceled. If so, log and return `t`.
...
(defun concur:cancel-token-cancel! (token)
  "Cancel the given TOKEN, mark it inactive, and invoke callbacks.

If the token is already inactive, this function does nothing. The
...
If TOKEN is already canceled, the CALLBACK is run immediately
(after a 0-second timer to avoid synchronous execution). Callbacks
are stored and checked for uniqueness using `eq`.

Arguments:
- `TOKEN` (concur-cancel-token): The token to monitor.
- `CALLBACK` (function): The zero-argument function to call upon cancellation.
...
If CALLBACK is nil, TOKEN and all its hooks are cleared. This is
for explicit cleanup if a token is no longer needed and has not
...
Returns:
`t` if a change was made (callback removed or token cleared),
`nil` otherwise."
  (unless (concur-cancel-token-p token)
    (error "Invalid cancel token: %S" token))
