...
;;; Commentary:
;;
;; This module provides the bridge between Emacs's built-in `coroutines` library
;; and the `concur` promise system. Its primary function, `concur:from-coroutine`,
...
- `:CANCEL-TOKEN` (concur-cancel-token, optional): A token to link to the
  promise's lifecycle for cancellation.
- `:MODE` (symbol, optional): The concurrency mode for the promise created
  by this coroutine. Defaults to `:deferred`.
             ...
             ((,driver (&optional ,resume-val)
                (condition-case err
                    (pcase (send! ,runner ,resume-val)
                      ...
                      ;; Case 2: Coroutine yields a plain value (ignored).
                      (,yielded-val
...
Returns:
  ...
  (if (functionp awaitable)
      (condition-case nil
          ...
          ;; concurrency mode, so we must use the default. Explicitly calling
          ;; `concur:from-coroutine` or `concur:async!` is the way to specify
          ;; a different mode.
          (concur:from-coroutine awaitable :mode :deferred)
        ;; If `concur:from-coroutine` fails, it's not a valid runner.
        (error nil))
    nil))
